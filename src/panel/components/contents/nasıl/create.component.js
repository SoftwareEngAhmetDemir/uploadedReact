...
export default function ContentCreate() {
  ...
  const onSubmit = async (e) => {
    ...
    if (gState.uploadPictures) {
      ...
      await axios
        .post(ENDPOINT+'/uploadimg/uploadimg/' + 'nasil', data)
        .then((res) => {
    ...
    axios.post(ENDPOINT+'/content/nasil/add', Content).then((res) => {
  ...
  return (
    <div className='containerP'>
      ...
      <Dialog disableBackdropClick disableEscapeKeyDown open={gropBoxOpen}>
        <DialogTitle>{t('Add New Content Name')}</DialogTitle>
        <DialogContent>
          <FormControl className='FormControl' style={{ width: '100%' }}>
            <InputLabel htmlFor='group'>{t('Add Content Name')}</InputLabel>
            <Input
              id='group'
              value={state.title}
              onChange={(e) => {
          ...
          <FormControl className='FormControl' style={{ width: '100%' }}>
            <InputLabel htmlFor='group'>{t('Content Description')}</InputLabel>
            <Input
              id='group'
              value={state.text}
              onChange={(e) => {
