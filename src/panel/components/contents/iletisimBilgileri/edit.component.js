...
export default function ContentEdit(props) {
  ...
  function getContentData() {
    axios
      .get(ENDPOINT+`/content/iletisim-bilgileri/${props.match.params.id}`)
      .then((response) => {
        setState({
          ...state,
  ...
  const onSubmit = async (e) => {
    ...
    if (gState.uploadPictures) {
      ...
      await axios
        .post(ENDPOINT+'/uploadimg/uploadimg/' + 'iletisimbilgileri', data)
        .then((res) => {
    ...
    axios
      .post(ENDPOINT+`/content/iletisim-bilgileri/${props.match.params.id}`, Content)
      .then((res) => {
        if (res.data.variant == 'error') {
          ...
          history.push('/panel/iletisimBilgilerilist');
        }
      })
  ...
  const deleteData = (id) => {
    axios.delete(ENDPOINT+`/content/iletisim-bilgileri/${id}`).then((res) => {
