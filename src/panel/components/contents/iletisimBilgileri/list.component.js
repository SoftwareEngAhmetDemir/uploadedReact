...
export default function ContentList() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (rowData) => (
        <div>
          ...
          <label style={{ margin: '5px' }}>({rowData.images.length})</label>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (rowData) => (
        <div>
          <Link to={`/panel/iletisimBilgileri/edit/${rowData._id}`}>
            <Edit />
          </Link>
        </div>
      ),
  ...
  const getContentData = () => {
    axios.get(ENDPOINT+'/content/iletisim-bilgileri').then((response) => {
              ...
              editable={{
                onRowAdd: (newData) =>
                  new Promise((resolve, reject) => {
                    axios
                      .post(ENDPOINT+'/content/iletisim-bilgileri/add', {
                        name: newData.name,
                      })
                      .then((response) => {
                ...
                onRowUpdate: (newData, oldData) =>
                  new Promise((resolve, reject) => {
                    axios
                      .post(ENDPOINT+`/content/iletisim-bilgileri/${newData._id}`, {
                        name: newData.name,
                      })
                      .then((response) => {
                ...
                onRowDelete: (oldData) =>
                  new Promise((resolve, reject) => {
                    axios
                      .delete(ENDPOINT+`/content/iletisim-bilgileri/${oldData._id}`)
                      .then((response) => {
                        const index = contentcategories.indexOf(oldData);
                ...
                components={{
                  Toolbar: (props) => (
                    <div>
                      ...
                      <Link
                        to='/panel/iletisimBilgilericreate'
