...
export default function ContentEdit(props) {
  ...
  function getContentData() {
    axios.get(ENDPOINT+`/content/bizkimiz/${props.match.params.id}`).then((response) => {
      setState({
        ...state,
  ...
  const onSubmit = async (e) => {
    ...
    if (gState.uploadPictures) {
      ...
      await axios
        .post('/uploadimg/uploadimg/' + 'bizkimiz', data)
        .then((res) => {
          imgpath = res.data.map((images) => {
            return images.path.replace('..\\public', '');
          });
          // delete old file
          axios.post('/uploadimg/removefile', { file: state.images });
        })
        .catch((err) => {
    ...
    axios
      .post(`/content/bizkimiz/${props.match.params.id}`, Content)
      .then((res) => {
        if (res.data.variant == 'error') {
          ...
          history.push('/panel/bizkimizlist');
        }
      })
  ...
  const deleteData = (id) => {
    axios.delete(ENDPOINT+`/content/bizkimiz/${id}`).then((res) => {
