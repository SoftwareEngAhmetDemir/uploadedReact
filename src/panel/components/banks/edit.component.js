...
export default function EstatePropsCreate(props) {
  ...
  function getBanksF() {
    axios.get(ENDPOINT+`/banks/${props.match.params.id}`).then((response) => {
      seTstate({
  ...
  const onSubmit = async (e) => {
    ...
    axios
      .post(ENDPOINT+`/banks/${props.match.params.id}`, sendData)
      .then((res) => {
        if (res.data.variant === "error") {
          enqueueSnackbar(t("Güncellenemedi") + res.data.messagge, {
            variant: res.data.variant,
          });
        } else {
          enqueueSnackbar(t("Güncellendi") + res.data.messagge, {
            variant: res.data.variant,
          });
          // navigate
