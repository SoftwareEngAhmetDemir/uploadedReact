...
export default function InvoicesList(props) {
  ...
  const columns = [
    ...
    {
      title: t('Banka Adı'),
      field: 'bank_id[0].label',
    },
    {
      title: t('Başlangıç Tarihi'),
      field: 'start_date',
    },
    {
      title: t('Bitiş Tarihi'),
      field: 'end_date',
    },

    {
      ...
      render: (rowData) => (
        <div>
          <Link to={`/panel/estate/edit/${rowData._id}`}>
            <Edit />
          </Link>
        </div>
      ),
  ...
  const getData = () => {
    axios.get(ENDPOINT+'/estate').then((response) => {
                ...
                components={{
                  Toolbar: (props) => (
                    <div>
                      ...
                      <Link to='/panel/estatecreate' className='addButtonPlace'>
