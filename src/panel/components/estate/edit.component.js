...
import SortableComponent from './imgOrder.components';
...
export default function EstateEdit(props) {
  ...
  const getData = async () => {
    await axios
      .get(ENDPOINT+`/estate/${props.match.params.id}`)
      .then(async (response) => {
        await seTdataProps(response.data.props);
        await seTgState({ ...gState, imagesOrder: response.data.images });
        await seTstate({
          ...state,
          no: response.data.no,
          title: response.data.title,
          created: response.data.created,
          start_price: response.data.start_price,
          now_price: response.data.now_price,
          part_price: response.data.part_price,
          ...
          selectedChance: response.data.chance,
          selectedDefaultBank: response.data.bank_id,
          ...
          selectedCategoryItems: response.data.chance_category
            ? response.data.chance_category
            : '',

          customersData: response.data.customersData
            ? response.data.customersData
            : '',
        });
      })
      .catch((err) => console.log(err));
  ...
  const getCustomers = () => {
    axios.get(ENDPOINT+'/staff/all/2').then((response) => {
      if (response.data.length > 0) {
        const details = [];
        for (const i in response.data) {
          details.push({
            label: response.data[i].name + ' ' + response.data[i].surname,
            value: [
              response.data[i].name + ' ' + response.data[i].surname,
              response.data[i]._id,
            ],
          });
        }
        seTcustomersData(details);
      }
    });
  };
  ...
  const seTorderImg = () => {
    if (gState.imagesOrder.length > 0) {
      seTstate({
        ...state,
        images: gState.imagesOrder,
      });
      seTopen(false);
    }
  };
  const onSubmit = async (e) => {
    ...
    if (gState.uploadPictures) {
      ...
      await axios
        .post('/uploadimg/uploadimg/' + 'estate', data)
        .then((res) => {
    ...
    if (gState.uploadPdf) {
      ...
      await axios
        .post('/uploadimg/uploadimg/' + 'estatepdf', data)
        .then((res) => {
          for (var i = 0; i < res.data.length; i++) {
    ...
    axios
      .post(ENDPOINT+`/estate/${props.match.params.id}`, Estate)
      .then((res) => {
        if (res.data.variant === 'error') {
      ...
      >
        <DialogContent style={{ padding: '0' }}>
          ...
          <h3 style={{ marginLeft: '25px' }}>
            Sıralamak için resimleri sürükleyin
          </h3>
          {state.images && state.images.length > 0 ? (
            <SortableComponent items={state.images} />
          ) : (
            ''
          )}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => seTorderImg(gState.imgOrder)} color='primary'>
            {t('okey')}
          </Button>
        </DialogActions>
      </Dialog>
                  ...
                  <FormGroup className='FormGroup'>
                    <FormControl>
                      <Select
                        ...
                        isMulti
                        placeholder={t(
                          'Bu ilana Teklif Verebilecek Müşteriler'
                        )}
                        value={state.customersData}
                        options={customersData}
                        onChange={(selectedOption) => {
                          seTstate({
                            ...state,
                            customersData: selectedOption,
                          });
                      ...
                      <FormHelperText>
                        {t('Burada yazılan üyeler bu ilana teklif verebilir ')}
                      </FormHelperText>
                    </FormControl>
                  </FormGroup>
                </Grid>
                              ...
                              {data.items.map((data2) => (
                                <FormControlLabel
                                  ...
                                  control={
                                    <Checkbox
                                      onChange={
                                        handleChangeEstateProps_checkbox
                                      }
                                      checked={
                                        dataProps
                                          ? dataProps[data.text]
                          ...
                          <FormGroup className='FormGroupAdress'>
                            <FormControl>
                              <TextValidator
                                ...
                                value={dataProps ? dataProps[data.text] : ''}
                                name={data.text}
